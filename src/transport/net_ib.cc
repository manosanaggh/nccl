...
static inline void ncclIbMeasureSendAccountStateLocked(uint64_t eventNs, bool accountOutstanding = true) {
  ...
  if (dt != 0) {
    ...
    if (ncclIbMeasureSendIdleOutstanding >= 2) ncclIbMeasureSendActiveOutstandingGe2Ns += dt;
...
static void ncclIbMeasureSendSummary() {
  ...
  uint64_t activeOutstandingGe2Ns = ncclIbMeasureSendActiveOutstandingGe2Ns;
  ...
  double activeOutstandingGe2ThroughputGBps = activeOutstandingGe2Ns == 0 ? 0.0 : (double)bytes / (double)activeOutstandingGe2Ns;
  ...
  INFO(NCCL_NET,
       "NET/IB: send measure summary count=%llu bytes=%llu wall_cq_s=%.6f active_outstanding_ge2_s=%.6f wall_cq_throughput_GBps=%.3f active_outstanding_ge2_throughput_GBps=%.3f",
       (unsigned long long)count, (unsigned long long)bytes, wallCqS, activeOutstandingGe2S,
       wallCqThroughputGBps, activeOutstandingGe2ThroughputGBps);
  INFO(NCCL_NET,
       ...
       (unsigned long long)buckets[10], (unsigned long long)buckets[11],
       (unsigned long long)buckets[12], (unsigned long long)buckets[13],
       (unsigned long long)buckets[14], (unsigned long long)buckets[15],
       (unsigned long long)buckets[16], (unsigned long long)buckets[17],
       (unsigned long long)buckets[18], (unsigned long long)buckets[19],
       (unsigned long long)buckets[20], (unsigned long long)buckets[21],
       (unsigned long long)buckets[22], (unsigned long long)buckets[23],
       (unsigned long long)buckets[24], (unsigned long long)buckets[25]);
}
...
static inline bool ncclIbMeasureSendIsTrackedBytes(uint64_t bytes) {
  int64_t minBytes = ncclParamIbMeasureSendMinBytes();
...
static inline void ncclIbMeasureSendComplete(struct ncclIbRequest* req, const char* path) {
  ...
  if (ncclParamIbMeasureSend()) {
    ...
    if (bucket >= ncclIbMeasureSendSummaryBucketCount) bucket = ncclIbMeasureSendSummaryBucketCount - 1;
    ncclIbMeasureSendSummaryThroughputBuckets[bucket].fetch_add(1, std::memory_order_relaxed);
  }
}
...
ncclResult_t ncclIbMultiSend(struct ncclIbSendComm* comm, int slot) {
  ...
  for (int i = 0; i < nqps; i++) {
...
#endif
    if (measureSend) {
      uint64_t postStartNs = ncclIbMeasureSendNowNs();
      if (measurePostStartNs == 0) {
        measurePostStartNs = postStartNs;
        for (int r=0; r<nreqs; r++) {
    ...
    if (measureSend) measurePostDoneNs = ncclIbMeasureSendNowNs();
  ...
  if (measureSend && measurePostDoneNs != 0) {
    for (int r=0; r<nreqs; r++) {
      ncclIbMeasureSendPostDone(reqs[r], measurePostDoneNs);
    }
  }
  ...
  while (1) {
    ...
    if (ncclIbRequestDone(r)) {
      ...
      *done = 1;
      ...
      if (r->type == NCCL_NET_IB_REQ_SEND) ncclIbMeasureSendComplete(r, "isend");
        ...
        for (int w=0; w<wrDone; w++) {
          ...
          if (req && req->type == NCCL_NET_IB_REQ_SEND) {
            for (int j = 0; j < req->nreqs; j++) {
              ...
              sendReq->events[i]--;
              if (ncclIbRequestDone(sendReq)) ncclIbMeasureSendComplete(sendReq, "cq-isend");
#ifdef NCCL_ENABLE_NET_PROFILING
                                ...
                                uint64_t dstOff, void *dstMhandle, uint32_t rank, void **request)
{
  ...
  ncclIbMeasureSendStart(req, size);
  struct ibv_send_wr* bad_wr;
  NCCLCHECK(wrap_ibv_post_send(qp->qp, &wr, &bad_wr));
  ncclIbMeasureSendPostDone(req, ncclIbMeasureSendNowNs());
  ncclIbAddEvent(req, qp->devIndex, &comm->devs[qp->devIndex].base);
...
ncclResult_t ncclGinIbProxyTest(void *collComm, void *request, int *done) {
  ...
  *done = 0;
  ...
  if (req->events[0] == 0) {
    *done = 1;
    ncclIbMeasureSendComplete(req, "gin_iput");
  ...
  for (int i = 0; i < wrDone; i++) {
    ...
    if (wcReq == req && wcReq->events[0] == 0) {
      *done = 1;
      ncclIbMeasureSendComplete(wcReq, "gin_iput");
